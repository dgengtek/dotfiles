...
class paste_symlink_relative(Command):
    """
    relative links even inside
    """
    def execute(self):
        import os
        from pathlib import Path

        old_dir = self.fm.thisdir
        cwd = os.path.abspath(".")
        cwd = os.path.realpath(cwd)
        selection = self.fm.copy_buffer
        os.chdir(cwd)

        for s in selection:
            try:
                relpath = os.path.realpath(s.path)
                relpath = os.path.relpath(relpath)
                os.symlink(relpath, s.basename)
