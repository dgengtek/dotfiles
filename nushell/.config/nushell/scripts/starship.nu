...
$env.PROMPT_MULTILINE_INDICATOR = (^/data/tmp/dgeng/cargo/bin/starship prompt --continuation)
...
$env.PROMPT_COMMAND = {
    ...
    ^/data/tmp/dgeng/cargo/bin/starship prompt $"--cmd-duration=($env.CMD_DURATION_MS)" $"--status=($env.LAST_EXIT_CODE)" $"--terminal-width=($width)"
}
