...
__find_get_tool() {
	for tool in "$@"; do
		if ! command -v "$tool" 2>&1 | logger -t bashrc -p user.info; then
			continue
		fi
		echo "$tool"
		break
...
find_dir_depth() {
	...
	local maxdepth=${3:-1}
	shift 3
	find "${path}" -mindepth "$mindepth" -maxdepth "${maxdepth}" -type d "$@"
}
find_dir_depth_prune_hidden() {
	local path=${1:-.}
	local mindepth=${2:-1}
	local maxdepth=${3:-1}
	shift 3
	find "${path}" -mindepth "$mindepth" -maxdepth "${maxdepth}" -not -path '*/\.*' -type d "$@"
}
...
find_file_depth() {
	...
	local maxdepth=${3:-1}
	shift 3
	find "${path}" -mindepth "$mindepth" -maxdepth "${maxdepth}" -type f "$@"
}
find_file_depth_prune_hidden() {
	local path=${1:-.}
	local mindepth=${2:-1}
	local maxdepth=${3:-1}
	shift 3
	find "${path}" -mindepth "$mindepth" -maxdepth "${maxdepth}" -not -path '*/\.*' -type f "$@"
}
